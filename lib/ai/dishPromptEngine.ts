// lib/ai/dishPromptEngine.ts
// Semantic prompt builder for high-quality, realistic luxury catering food photography.
// Enforces strict dietary isolation, culinary authenticity, and premium styling.
...
// ─── Dietary & Culinary Guard Rules ──────────────────────────────────────────

const DIETARY_NEGATIVE_GUARDS: Record<string, string[]> = {
  VEG: [
...
const FAMILY_NEGATIVE_GUARDS: Record<string, string[]> = {
...
/**
 * Normalizes text for clean semantic matching
 */
...
/**
 * Detects culinary family and protein/substance profile from dish metadata
 */
export function analyzeDishProfile(dish: Pick<Dish, 'name' | 'description' | 'dietary' | 'cuisine_tags' | 'course_tags'>) {
  ...
  // Check course tags & food category markers
  ...
  // Protein / ingredient family analysis
...
/**
 * Builds a structured, semantically strict prompt from dish data.
 * Adheres to:
 * - Real dish data (no hallucinated ingredients)
 * - Strict dietary constraints (no meat on veg, no seafood cross-contamination)
 * - Luxury catering food photography visual style
 * - Natural textures, elegant plating, editorial lighting, clean composition
 * - Absolute negative constraints (no text, no people, no watermarks)
 */
export function buildDishImagePrompt(dish: Dish): GeneratedPromptResult {
  ...
  // Safe description incorporation (avoiding run-on noise)
  ...
  // Assemble comprehensive negative prompt
  const baseNegatives = [
